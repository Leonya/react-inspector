...
function getOwnerIframe(node: HTMLElement): HTMLIFrameElement | null {
  const nodeWindow = getOwnerWindow(node);
  if (nodeWindow) {
    return nodeWindow.frameElement as HTMLIFrameElement;
  }
  return null;
}
...
function getNestedBoundingClientRect(
  ...
  if (ownerIframe && (ownerIframe as unknown) !== boundaryWindow) {
    const rects = [node.getBoundingClientRect()] as Array<Rect>;
...
function findTipPos(dims: Box, bounds: Box, tipSize: Box) {
